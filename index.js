...
for (let idx = 0; idx < process.argv.length; ++idx) {
    ...
    if (value.indexOf("--help") > -1 || value.indexOf("-h") > -1) {
        hasHelp = true;
        break;
    }

    if (value.indexOf("--platform=") > -1) {
        platform = value
            .split("=")
            .pop()
            .replace("\"", "")
...
{
    execSync(
        `${process.cwd()}/node_modules/.bin/webpack --mode production --env=${environment} --platform=${platform}`
    );
};
...
const setElectronIndexPath = () =>
{
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            "path":  htmlFilePath.replace(process.cwd(), ".")
        }, null, 2)
    );
};
...
switch (platform) {
    ...
    default:
        console.log();
        console.log("`--platform` can be specified for macOS, Windows, iOS, Android, Web, and Steam.");
